// Importaciones de terceros sin cambios
import { createContext, useEffect, useRef, useState } from 'react'
import { PropTypes } from 'prop-types'
import * as Yup from 'yup'
import { toast } from 'react-hot-toast'

// Servicios - usar alias @api
import {
  getVideos,
  getCategorias,
  createVideo,
...
import { generatePDF, generateRecommendations } from '@/api/services/libraryServices'
...
export const VideosProvider = ({ children }) => {
  ...
  const fetchInitialData = async () => {
    try {
      setLoading(true)
      const [videosRes, categoriasRes] = await Promise.all([getVideos(), getCategorias()])
  ...
  const toggleFavorite = videoId => {
    setFavorites(prev => {
      ...
      const newFavorites = isFavorite ? prev.filter(id => id !== videoId) : [...prev, videoId]
  ...
  const getHistoryVideos = () => {
    return viewHistory.map(id => videos.find(video => video.id === id)).filter(Boolean)
  }
  ...
  const updateRecommendations = (currentVideos = videos) => {
    const recommendations = generateRecommendations({ viewHistory, favorites }, currentVideos)
