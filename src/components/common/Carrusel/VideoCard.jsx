    ...
    <VideoBox color={color}>
      <VideoCardMain>
        <Box sx={{ position: 'relative' }}>
          <VideoCardImg component='img' image={video.linkImage} alt={video.titulo} />
          <OverlayContainer>
          ...
          <ActionButtons direction='column' spacing={1}>
            {user && (
              <Tooltip title={isFavorite ? 'Quitar de favoritos' : 'Añadir a favoritos'}>
                <IconButton onClick={handleFavoriteClick} sx={{ color: colorWhite }}>
                  {isFavorite ? <FavoriteIcon /> : <FavoriteBorderIcon />}
                </IconButton>
              </Tooltip>
            )}
            <Tooltip title='Compartir'>
              <IconButton onClick={handleShare} sx={{ color: colorWhite }}>
                <ShareIcon />
              </IconButton>
            </Tooltip>
          </ActionButtons>
        ...
        <VideoCardText>
          ...
          {user && (
            <>
              ...
              />
              <CourseProgress videoId={video.id} progress={progress} duration={video.duracion} />
            </>
          )}
        </VideoCardText>
      </VideoCardMain>
