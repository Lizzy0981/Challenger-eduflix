...
function DrawerComponent() {
  ...
  return (
    <>
      ...
      <DrawerBox anchor='right' open={open} onClose={toggleDrawer}>
        <DrawerHeader>
        ...
        <UserInfo>
          <Avatar alt='User Profile' src='/placeholder-avatar.png' sx={{ width: 40, height: 40 }} />
          <Box>
        ...
        <List sx={{ '&': { [animations.slideIn.split(';')[0]]: '' } }}>
          {menuItems.map(item => (
            <ListItemStyled key={item.text} component={Link} to={item.path} onClick={toggleDrawer}>
              <ListItemIcon>{item.icon}</ListItemIcon>
              <ListItemText primary={item.text} sx={{ color: colorWhite }} />
