...
// Importamos Boton desde common (subimos un nivel con ../)
import Boton from '../Button/Boton'

// Importamos los estilos desde el archivo local
import {
  ButtonContainer,
  ButtonLeft,
  ButtonRight,
  ...
  return (
    <>
      <Form onSubmit={formik.handleSubmit}>
        <Titulo variant='h4' component='h2'>
          Nueva Categoría
        </Titulo>
        <TextField
          label='Nombre'
          ...
          <Head>
            <TableRow>
              {headerTable.map((header, index) => (
                <TableCellHeader align='center' key={index}>
                  {header}
                </TableCellHeader>
              ))}
            </TableRow>
          ...
          <TableBody>
            {categorias?.map(categoria => (
              <TableRowStyled key={categoria.id}>
                ...
                <TableCellBody align='center'>
                  <Button onClick={handleDeleteCategoria} color='error'>
                    <Delete />
                  </Button>
                </TableCellBody>
              </TableRowStyled>
