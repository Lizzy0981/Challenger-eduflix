...
import { Box, Typography, Tabs, Tab, Grid, Button, Fade, Divider, Tooltip } from '@mui/material'
...
function UserLibrary() {
          ...
          {filteredVideos.map(video => (
            <Grid item xs={12} sm={6} md={4} lg={3} key={video.id}>
              <VideoCard
                video={video}
                color={
                  categorias.find(cat => cat.nombre === video.categoria)?.color || colorPrimary
                }
  ...
  return (
    <LibraryContainer>
      ...
      <ActionsContainer>
        ...
        <Tooltip title='Ver cursos recomendados'>
          <ActionButton
            ...
            onClick={() => {
              /* Implementar vista de recomendaciones */
            }}
          >
            Recomendaciones
          </ActionButton>
      ...
      >
        <StyledTab icon={<FavoriteIcon />} label='Favoritos' />
        <StyledTab icon={<HistoryIcon />} label='Historial' />
        <StyledTab icon={<QueryStatsIcon />} label='Estadísticas' />
      </Tabs>

      {currentTab !== TAB_VALUES.STATS && <LibraryFilters onFilterChange={setFilters} />}
