...
// Necesitamos acceder a los otros modelos a través de mongoose.model
...
progressSchema.methods.updateProgress = async function (currentTime, duration) {
  ...
  if (timeDiff > 0 && timeDiff < 3600) {
    // Ignorar diferencias mayores a 1 hora
  ...
  if (this.completed) {
    ...
    const [video, user] = await Promise.all([Video.findById(this.video), User.findById(this.user)])
