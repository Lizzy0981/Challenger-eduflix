// Actualizamos las importaciones con alias
import Category from '@api/models/Category'
import Video from '@api/models/Video'
import { createError } from '@api/middleware/errorHandler'
import { validateCategoryData } from '@api/utils/validators'

export const getCategories = async (req, res, next) => {
  try {
    const { sort = 'orden', order = 'asc', activa } = req.query
    ...
    const categories = await Category.find(query).sort(sortOptions).populate('videos')
...
export const updateCategoryOrder = async (req, res, next) => {
  try {
    ...
    await Promise.all(
      orderedIds.map((id, index) => Category.findByIdAndUpdate(id, { orden: index }))
    )
