import { Video, Progress } from '@api/models'
import { createError } from '@api/middleware/errorHandler'
import { validateVideoURL, getVideoDuration } from '@api/utils/validators'
import { notificationService } from '@api/services/notificationService'

class VideoService {
  async getVideos(filters = {}, pagination = {}) {
    const { categoria, nivel, duracion, instructor, search } = filters
  ...
  async rateVideo(videoId, userId, rating, comment = '') {
    ...
    const existingRating = video.ratings.find(r => r.user.toString() === userId)
