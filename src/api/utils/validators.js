...
export const videoSchema = Joi.object({
  ...
  linkVideo: Joi.string().uri().required().messages({
    'string.uri': 'URL del video inválida',
    'any.required': 'El link del video es obligatorio'
  }),

  linkImage: Joi.string().uri().required().messages({
  ...
  descripcion: Joi.string().min(20).max(1000).required().messages({
  ...
  codigoSeguridad: Joi.string().required().messages({
...
export const categorySchema = Joi.object({
  ...
  descripcion: Joi.string().min(20).max(500).required().messages({
    'string.min': 'La descripción debe tener al menos 20 caracteres',
